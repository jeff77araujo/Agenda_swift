...
class MenuOpcoesAlunos: NSObject {
    ...
    func configuraMenuDeOpcoesDoAluno(_ alunoSelecionado: Aluno, navigation: UINavigationController) -> UIAlertController {
        ...
        let waze = UIAlertAction(title: "localizar no waze", style: .default) { (acao) in
            Localizacao().localizaAlunoWaze(alunoSelecionado)
        }
        menu.addAction(waze)
        ...
        let abrirPaginaWeb = UIAlertAction(title: "abrir página", style: .default) { (acao) in
            Safari().abreWeb(alunoSelecionado, controller: viewController)
        }
