...
class CalculaMediaAPI: NSObject {
    ...
    func calculaMediaGeralDosAlunos(alunos: [Aluno], sucesso:@escaping(_ dicionarioDeMedias: [String:Any]) -> Void, falha:@escaping(_ error:Error) -> Void) {
        
        guard let url = URL(string: "https://www.caelum.com.br/mobile") else { return }
        var listaDeAlunos: [[String:Any]] = []
        var json: [String:Any] = [:]
        ...
        do {
            ...
            let task = URLSession.shared.dataTask(with: requisicao, completionHandler: { (data, response, error) in
                if error == nil {
                    do {
                        let dicionario = try JSONSerialization.jsonObject(with: data!, options: []) as! [String:Any]
                        sucesso(dicionario)
                    } catch {
                        falha(error)
                    }
