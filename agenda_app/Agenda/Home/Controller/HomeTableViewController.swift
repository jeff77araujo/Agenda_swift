...
class HomeTableViewController: UITableViewController, UISearchBarDelegate {
    ...
    func recuperarAlunos() {
        Repository().recuperarAlunos { (listaAlunos) in
    ...
    @objc func abrirActionSheet(_ longPress: UILongPressGestureRecognizer) {
        if longPress.state == .began {
            ...
            let menu = MenuOpcoesAlunos().configuraMenuDeOpcoesDoAluno(alunoSelecionado, navigation: navigation)
            ...
            AutenticacaoLocal().autorizaUsuario(completion: { (autenticado) in
                if autenticado {
                    DispatchQueue.main.async {
                        ...
                        Repository().deletarAluno(aluno: alunoSelecionado)
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        if let texto = searchBar.text {
            alunos = Filter().filtrarAlunos(listaAlunos: alunos, texto: texto)
        }
